...
- [x] **M1. 진짜 데이터베이스** ✅ (선생님 버전 완료) — 파일 JSON → Prisma+Postgres(Neon).
  **인터페이스 고정 전환의 실물**: store.ts 함수 19개 시그니처 무변경, 호출부 0줄 수정으로 빌드 통과.
  검증: 새 프로세스 영속성·DB unique 제약(P2002)·실제 가입 API→Neon 왕복.
  네 주석("Prisma 임시 대체, 호출부 무변경 전제")이 설계도가 됐다 — 그 주석이 이 작업의 8할
- [ ] **M2. 회원·결제 완성** — 실패·취소 화면, 웹훅 서명 검증·멱등성, 만료 토큰 처리
- [x] **M3. 세상에 공개** ✅ (선생님 버전 완료) — **https://first-commit-teacher.vercel.app**
  전 페이지(홈·/world·/nft·/gallery) 동작, 프로덕션 가입→Neon DB 왕복 검증.
  배포 시 마이그레이션 자동 적용(vercel-build). 남은 키 입력(학생/강사 계정 소유):
  Resend(실메일 발송 — 지금은 로그 폴백으로 가입 동작), Stripe 키+웹훅 등록, Discord OAuth
- [ ] **M4. 까비 생태계 연결** — `/world` 임베드(까비 월드), NFT 갤러리(OpenSea)
  - 🔄 **예고편 완성** (선생님 버전): `/world` 페이지가 까비 월드를 임베드하고 로그인 닉네임을 postMessage로 전달 — 월드 쪽 수신부(출처 검증·멱등)까지 양방향 구현·검증됨. 남은 것: 배포 후 실세션 연동(M3 뒤)
  - 덤: 프로덕션 빌드를 막던 기존 버그(checkout/success의 useSearchParams Suspense 누락)를 선생님 버전에서 수정 — M3 때 만났을 벽을 미리 제거
  - 🔄 **메타버스 1번 묶음 완성** (선생님 버전): ① 라이브 위젯 — 홈이 월드 P2P 방에 관전자로 접속해 "지금 월드에 N명 — 닉네임" 실시간 표시 (서버 0대) ② `/nft` 갤러리 — 코덱스 캐릭터 카드+OpenSea 연결 ③ 입장 포탈 연출. 다음: M1(DB) 후 아바타 프로필·지갑 L1
  - 🖼 **3D 메타버스 전시관 `/gallery`** (선생님 버전): R3F로 우주 전시관 — 캐릭터 11종이 네온 액자로 걸려 있고 WASD로 걸어다니며 관람, 바라본 액자 정보+OpenSea 링크. 이미 깔려 있던 R3F를 본격 활용한 사례. 디버깅 교훈: drei Text가 three 0.184에서 컨텍스트 크래시 → 캔버스 텍스처로 우회 (참고: uwemaurer/nft-metaverse)
